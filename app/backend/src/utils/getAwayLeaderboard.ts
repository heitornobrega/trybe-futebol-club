import { IFinalizedMatches, ILeaderBoard } from './getHomeLeaderboard';

const totalPoints = (finalizedMatches: IFinalizedMatches[], name: string) => {
  const awayTeams = finalizedMatches.filter(({ teamAway: { teamName } }) => teamName === name);
  let points = 0;
  const draws = awayTeams.reduce((acc, curr) => {
...
const totalGames = (finalizedMatches: IFinalizedMatches[], name: string) =>
  finalizedMatches.filter(({ teamAway: { teamName } }) => teamName === name).length;

const totalDraws = (finalizedMatches: IFinalizedMatches[], name: string) => {
  const homeTeams = finalizedMatches.filter(({ teamAway: { teamName } }) => teamName === name);
  let draw = 0;
...
const goalsFavor = (finalizedMatches: IFinalizedMatches[], name: string) => {
  const homeTeams = finalizedMatches.filter(({ teamAway: { teamName } }) => teamName === name);
  const homeGoals = homeTeams.reduce((acc, curr) => acc + curr.awayTeamGoals, 0);
  return homeGoals;
};

const goalsOwn = (finalizedMatches: IFinalizedMatches[], name: string) => {
  const homeTeams = finalizedMatches.filter(({ teamAway: { teamName } }) => teamName === name);
  const awayGoals = homeTeams.reduce((acc, curr) => acc + curr.homeTeamGoals, 0);
  return awayGoals;
};
...
const getAwayLeaderboard = (finalizedMatches: IFinalizedMatches[]) => {
  const teams = [...new Set(finalizedMatches.map((item) => item.teamAway.teamName))];
...
export default getAwayLeaderboard;
